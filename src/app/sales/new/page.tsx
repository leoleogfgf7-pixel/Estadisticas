...
interface Product {
  id: string; name: string; cost: string; priceRetail: string;
  priceWholesale: string; priceSpecial: string; stock: number; active?: boolean;
}

interface Client {
  id: string; name: string; documentNumber: string | null;
}

interface CartItem {
  productId: string; name: string; cost: number; price: number;
  quantity: number; subtotal: number; profit: number;
}
...
export default function NewSalePage() {
  ...
  const getPrice = useCallback((product: Product) => {
    switch (saleType) {
      case "wholesale": return parseFloat(product.priceWholesale);
      case "special": return parseFloat(product.priceSpecial);
      default: return parseFloat(product.priceRetail);
    }
  }, [saleType]);
  ...
  const addToCart = (product: Product) => {
    ...
    setCart([...cart, {
      ...
      quantity: 1,
      subtotal: price,
      profit: price - parseFloat(product.cost),
    }]);
    setSearchTerm("");
  };
  ...
  const total = subtotal; // Tax could be added here
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/sales", {
      ...
      body: JSON.stringify({
        ...
        notes: notes || null,
        dueDate: paymentMethod === "credit" ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0] : null,
        items: cart.map(item => ({
    ...
    if (res.ok) {
      const data = await res.json();
      setSuccess(`Venta #${data.sale.id.slice(0, 8)} registrada exitosamente`);
      setCart([]);
      setClientId("");
      setNotes("");
      fetchProducts();
  ...
  return (
    <AppLayout>
      <div className="animate-fade-in space-y-6">
        ...
        {error && (
          <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl p-4 text-red-700 dark:text-red-400">{error}</div>
        )}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
              <div className="flex flex-wrap gap-3 mb-4">
                {(["retail", "wholesale", "special"] as const).map(type => (
                  ...
                  >
                    {type === "retail" ? "Por Menor" : type === "wholesale" ? "Por Mayor" : "Especial"}
                  </button>
                ))}
              </div>
