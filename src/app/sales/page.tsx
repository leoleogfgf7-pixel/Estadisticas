...
interface Sale {
  ...
  saleType: string; paymentMethod: string; paymentStatus: string;
...
export default function SalesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(`⚠️ ¿Anular esta venta?\n\nEl stock será devuelto a los productos.\nEsta acción se registra en el log de actividad.`)) return;
          ...
          {(["today", "week", "month", "all"] as const).map(f => (
            <button key={f} onClick={() => setDateFilter(f)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                dateFilter === f ? "bg-amber-500 text-white" : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300"
              }`}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={9} className="px-4 py-16 text-center text-slate-400">
                    ...
                    <p className="mt-2 font-medium text-slate-600 dark:text-slate-300">Sin ventas en este periodo</p>
                    <p className="text-sm">Ve a "Nueva Venta" para registrar tu primera venta</p>
                  </td></tr>
                ) : (
                  sales.map((s) => (
                    <tr key={s.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-xs whitespace-nowrap">{formatDateTime(s.createdAt)}</td>
                      <td className="px-4 py-3 text-slate-900 dark:text-white font-medium">{s.clientName || "General"}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <a href={`/api/sales/${s.id}/pdf`} target="_blank" className="text-blue-500 hover:text-blue-600 text-xs font-medium">📄 PDF</a>
                          <button onClick={() => handleDelete(s.id)} className="text-red-500 hover:text-red-600 text-xs font-medium">🗑️ Anular</button>
                        </div>
                      </td>
                    </tr>
                  ))
